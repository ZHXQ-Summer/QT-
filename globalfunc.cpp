...
std::vector<std::vector<ItemPost>::iterator> findPost(QString title="",
    ...
    for (auto it = posts.begin(); it != posts.end(); ++it) {
        ...
        // 检查标签
        if (!it->getFlags().empty()) {
            for (auto flag : it->getFlags()){
                if(flag.contains(title,Qt::CaseInsensitive)){
                    match = true;
                }
            }
        }
        if(!title.isEmpty() && it->getDescription().contains(title,Qt::CaseInsensitive)){
            match = true;
        }
        if (match) {
            result.push_back(it);
